                    ...
                    <tbody>
                        ...
                        @foreach($paginatedCalendars as $date => $group)
                            @php
                                ...
                                $dateOnly = (count($dateParts) === 4) 
                                    ? implode('-', array_slice($dateParts, 1)) 
                                    : $date;
                            ...
                            {{-- @dd($calendarGroup) --}}
                                @php
                                    $calendars = $calendarGroup['calendars_by_stadium'];
                                    ...
                                    <tr>
                                        ...
                                        @foreach ($calendarGroup['calendars'] as $calendar)
                                            ...
                                            @foreach ($calendar->courseStudents as $students)
                                                {{-- Thông tin học viên --}}
                                                <td class="text-nowrap"> {{ $students->student->name }}</td>
                                                <td class="text-nowrap">{{ !empty($students->student->dob) ? \Carbon\Carbon::parse($students->student->dob)->format('d-m-Y') : '--' }}</td>
                                                <td>{{ $students->student->identity_card ?? '--' }}</td>
                                                <td>{{ $students->student->phone ?? '--' }}</td>
                                                <td>{{ $students->pivot->exam_number ?? '--' }}</td>
                                                <td class="text-nowrap">
                                                    <span class="badge bg-success">{{ $students->course->code ?? '--' }}</span>
                                                </td>
                                                <script>
                                            ...
                                            @foreach ($calendar->courseStudents as $students)
                                                {{-- Thông tin học viên --}}
                                                <td class="text-nowrap">{{ !empty($students->health_check_date) ? \Carbon\Carbon::parse($students->health_check_date)->format('d-m-Y') : '--' }}</td>
                                            @endforeach
                                            <td class="text-nowrap">
                                            ...
                                            <td>
                                                @if (!empty($students->pivot->pickup) && $students->pivot->pickup == 1)
