...
<div class="container">
    ...
    <div class="card bg-white p-6 rounded-2xl shadow-lg">
        ...
        <p><strong>Loại khóa học:</strong> {{ $course->rank }}</p>
        <p><strong>Nhóm cấp bậc khóa học:</strong> {{ $course->rank_gp ?? 'N/A' }}</p>
        <p><strong>Số hồ sơ báo cáo:</strong> {{ ucfirst($course->number_bc) ?? 'N/A' }}</p>
        <p><strong>Ngày bắt đầu báo cáo:</strong> {{ $course->date_bci ?? 'N/A' }}</p>
        <p><strong>Ngày bắt đầu:</strong> {{ $course->start_date ?? 'N/A' }}</p>
        <p><strong>Ngày kết thúc:</strong> {{ $course->end_date ?? 'N/A' }}</p>
        ...
        <p><strong>Trạng thái:</strong>
                <span class="badge bg-info">{{ ucfirst($course->status) }}</span>
        </p>
    </div>
