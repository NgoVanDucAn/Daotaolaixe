...
{{-- @once
    @php
        function renderExamColumns($examsGroup, $requiredExamIds, $examResults, $student, $course) {
            $html = '';

            if ($examsGroup->isNotEmpty()) {
                foreach ($examsGroup as $exam) {
                    ...
                    if ($isRequired) {
                        ...
                        $html .= '<button class="btn btn-sm btn-link p-0 m-0 open-exam-modal"
...
@section('page-action-add-button')
    <div class="d-flex justify-content-between align-items-center">
        ...
        >
            <span style="font-size: 20px; margin:0 4px 2px 0;">+</span> <span>Thêm học viên khóa học ô tô</span>
        </a>
    </div>
@endsection
...
<div class="card">
    <div class="card-body">
        ...
        <form method="GET" action="{{ route('students.index-car') }}" class="row">
            {{-- <div class="col-12 col-md-4 mb-2 row"> --}}
                ...
                <div class="row">
                    <div class="col-12 col-sm-6 col-md-3 mb-3">
                        <label class="form-label fw-bold">Họ tên / Mã ĐK / Số CMT</label>
                        <input type="text" class="form-control" name="q" value="{{ request('q') }}" placeholder="Nhập thông tin" />
                    </div>
                    {{-- <div class="col-12 col-sm-6 col-md-3 mb-3">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <optgroup label="Thi">
                                ...
                                <option value="2" {{ request('status') == '2' ? 'selected' : '' }}>Đỗ</option>
                                <option value="3" {{ request('status') == '3' ? 'selected' : '' }}>Trượt</option>
                                <option value="4" {{ request('status') == '4' ? 'selected' : '' }}>Thi lại</option>
                                <option value="5" {{ request('status') == '5' ? 'selected' : '' }}>Thi mới</option>
                                <option value="6" {{ request('status') == '6' ? 'selected' : '' }}>Bỏ thi</option>
                                <option value="7" {{ request('status') == '7' ? 'selected' : '' }}>Hoãn lại</option>
                            </optgroup>
                            <optgroup label="Học">
                                <option value="8" {{ request('status') == '8' ? 'selected' : '' }}>Đang chờ</option>
                                <option value="9" {{ request('status') == '9' ? 'selected' : '' }}>Đang học</option>
                                <option value="10" {{ request('status') == '10' ? 'selected' : '' }}>Hoàn thành</option>
                                <option value="11" {{ request('status') == '11' ? 'selected' : '' }}>Thiếu giáo viên</option>
                                <option value="12" {{ request('status') == '12' ? 'selected' : '' }}>Hủy ca</option>
                                <option value="13" {{ request('status') == '13' ? 'selected' : '' }}>Hoãn</option>
                                <option value="14" {{ request('status') == '14' ? 'selected' : '' }}>Bỏ ca lại</option>
                            </optgroup>
                        </select>
                    </div> --}}
                    <div class="col-12 col-sm-6 col-md-3 mb-3">
                    ...
                    <div class="col-12 col-sm-6 col-md-3 mb-3">
                        <label class="form-label fw-bold">Trạng thái học phí</label>
                        <select name="status_fee" class="form-select">
                            <option value="">-- Chọn trạng thái --</option>
                            <option value="0" {{ request('status_fee') === '0' ? 'selected' : '' }}>Đã đóng đủ</option>
                            <option value="1" {{ request('status_fee') === '1' ? 'selected' : '' }}>Chưa đóng đủ</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="col-12 col-lg-3 col-md-12 mb-2 mt-2">
                <div class="d-flex align-items-center">
                    ...
                    <div class="ms-2">
                        <a href="{{ route('students.index-car') }}" class="btn btn-outline-danger mb-1" id="clearFilters"><b><i class="mdi mdi-sync me-1"></i>Refresh</b></a>
                    </div>
                </div>
            </div>
        </form>
            ...
            <table class="table mt-3 table-bordered">
                <thead>
                    <tr>
                        ...
                        {{-- <th rowspan="2" class="text-nowrap align-middle">Hạng đăng ký</th> --}}
                    ...
                    <tr>
                        <th rowspan="2" class="align-middle" style="min-width: 120px">Thi hết môn LT</th>
                        <th rowspan="2" class="align-middle" style="min-width: 120px">Thi hết môn TH</th>
                        <th rowspan="2" class="align-middle" style="min-width: 120px">Thi tốt nghiệp</th>
                ...
                <tbody>
                    ...
                    @foreach ($students as $student)
                        ...
                        @if($courseCount > 0)
                            @foreach ($student->courseStudents as $course)
                                <tr class="student-main-row" data-student-id="{{ $student->id }}">
                                    <td>{{ ++$key }}</td>
                                    {{-- <td>
                                        <span class="badge {{ isset($student->ranking->name) ? 'bg-success' : 'bg-secondary' }}">
                                            {{ ucfirst($student->ranking->name ?? 'Không') }}
                                        </span>
                                    </td> --}}
                                        ...
                                        @if ($course->course->id == 99999999)
                                            <a href="{{ route('student.course.action', ['student' => $student->id, 'course' => 99999999]) }}">
                                                <span class="badge bg-success">
                                                    ...
                                                    @else
                                                        {{ ucfirst($course->course->code) }}
                                                    @endif
                                                </span>
                                            </a>
                                        @else
                                        <script>
                                            console.log(111111, @json($course->course->id));
                                            console.log(2222222,@json($student->id));

                                        </script>
                                            <a href="{{ route('student.course.action', ['student' => $student->id, 'course' => $course->course->id]) }}">
                                                <span class="badge bg-success">{{ ucfirst($course->course->code) }}</span>
                                            </a>
                                        @endif
                                    ...
                                    <td>
                                        <span class="badge bg-success">
                                            ...
                                            {{ ucfirst($course->course->ranking->name ?? "") }}
                                        </span>
                                    </td>
                                    {{-- <td>
                                        @php
                                    ...
                                    <td>{{ $student->phone }}</td>
                                    <td>
                                        {{ \Carbon\Carbon::parse($course->course->contract_date)->format('d/m/Y') }}
                                    </td>
                                    ...
                                    {{-- ===== TÌNH TRẠNG THI (LT / TH / TN) từ $examsResults ===== --}}
                                    @php
                                    ...
                                    <td>
                                        @if(isset($remainingFees[$student->id][$course->course->id]))
                                            {{ number_format($course->tuition_fee ?? 0, 0, ',', '.') }} VND
                                        @else
                                            0 VND
                                        @endif
                                    </td>
                                    <td>
                                        @if(isset($remainingFees[$student->id][$course->course->id]))
                                            {{ number_format($courseFees[$student->id][$course->course->id] ?? 0, 0, ',', '.') }} VND
                                        @else
                                            0 VND
                                        @endif
                                    </td>
                                    <td>
                                        @if(isset($remainingFees[$student->id][$course->course->id]))
                                            <span class="badge {{ ($remainingFees[$student->id][$course->course->id] ?? 0) > 0 ? 'bg-danger' : 'bg-success' }}">
                                                {{ number_format($remainingFees[$student->id][$course->course->id] ?? 0, 0, ',', '.') }} VND
                                            </span>
                                        @else
                                    ...
                                    <td>
                                                ...
                                                {{ $course->course->status == 0 ? 'Chưa học' : ($course->course->status == 1 ? 'Đang học' : ($course->course->pivot->status == 2 ? 'Bỏ học' : 'Đã tốt nghiệp')) }}
                                                </span>
                                            </td>
                                    <td class="text-nowrap">
                                        <a
                                        ...
                                        <a
                                            href="{{ route('student.course.edit', ['student' => $student->id, 'course' =>  $course->course->id]) }}"
                        ...
                        <tr class="student-main-row" data-student-id="{{ $student->id }}">
                            ...
                            <td>{{ $student->phone }}</td>
                            <td>{{ $student->date_of_profile_set ? $student->date_of_profile_set->format('d-m-Y') : '' }}</td>
                            <td class="text-center">{{ $student->identity_card }}</td>
                            <td></td>
                            {{-- <td class="text-nowrap text-start">{{ $student->address }}</td> --}}
                            <td>
                                ...
                                @foreach($student->courses as $course)
                                    ...
                                    <td>
                                        @if(isset($remainingFees[$student->id][$course->id]))
                                            {{ $course->pivot->tuition_fee ? number_format($course->pivot->tuition_fee, 0, ',', '.') : 0 }} VND</td>
                                        @else
                                            0 VND
                                        @endif
                                    </td>
                            ...
                            <td class="text-nowrap">
                                <a href="javascript:void(0)"
                                    class="btn btn-sm btn-primary btn-add-student-course"
                                    data-bs-toggle="modal"
                                    data-bs-target="#addStudentModal"
                                    data-add-student="true"
                            ...
                            <div class="col-md-9">
                                <div class="row">
                                    <div class="col-12 col-md-6 mb-3">
                                        ...
                                        {{-- <input type="text" name="" value="{{ $student->name }}"> --}}
                                        {{-- <input type="hidden" name="student_id" value="{{ $student->id }}"> --}}
                                        <select name="student_id" id="add_student_id" class="form-select" required>
                                    ...
                                    <div class="col-12 col-md-6 mb-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" placeholder="HH:mm" name="give_chip_hour" id="give_chip_hour" class="form-control time-input" value="{{ old('give_chip_hour') }}">
                                        </div>
                                    </div>
                                    <div class="col-12 col-md-6 mb-3">
                                        <div class="form-group">
                                            <label for="order_chip_hour">Số giờ đặt chip</label>
                                            <input type="text" placeholder="HH:mm" name="order_chip_hour" id="order_chip_hour" class="form-control time-input" value="{{ old('order_chip_hour') }}">
                                        </div>
                                    </div>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div id="exam-fields" class="mb-3" style="display: none;">
                                    ...
                                    <div class="form-group mb-3 position-relative">
                                        ...
                                        <input type="text" autocomplete="off" placeholder="dd/mm/yyyy"name="exam_fee_deadline" id="exam_fee_deadline" class="form-control real-date @error('exam_fee_deadline') is-invalid @enderror"/>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="updateCardForm">
                        @csrf
                        <input type="hidden" name="modal_name" value="updateCardModal">
...
<script>
    ...
    function loadCourses() {
        return fetch(`/listCourse?vehicle_type=2`)
            .then(response => response.json())
            .then(courses => {
