...
class CalendarController extends Controller
{
    ...
    public function index2(Request $request, $type = null, $level_filter = null)
    {
        ...
        if ($request->filled('start_date')) {
            $dateBreadscrum = $request->start_date;
            $breadcrumb = $request->attributes->get('breadcrumb');
            $originalTitle = $request->attributes->get('page_title');
            $breadcrumb->add('Ngày ' . \Carbon\Carbon::parse($dateBreadscrum)->format('d/m/Y'));
            $pageTitle = $originalTitle . ' ngày ' . \Carbon\Carbon::parse($dateBreadscrum)->format('d/m/Y');
            view()->share(['breadcrumb', $breadcrumb,'page_title' => $pageTitle,]);
        }
        if ($request->filled('start_date') || $request->filled('end_date')) {
            ...
            if ($start && $end) {
                ...
                if ($request->filled('time')) {
                    $query->whereDate('date_start', $start ?? $end)->where('time', $request->time);
                } else {
                    $query->whereDate('date_start', $start ?? $end);
                }
            }
